...
function Index() {
  ...
  const field = "h-12 w-full rounded-xl border border-white/10 bg-white/[.07] px-4 text-sm text-white placeholder:text-zinc-400 outline-none transition focus:border-red-500 focus:bg-white/[.1]";
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-950 antialiased">
      ...
      <main>
        ...
        <section id="steel-frame" className="relative overflow-hidden bg-[linear-gradient(135deg,#8f0d14_0%,#c9141c_48%,#ef2931_100%)] py-20 text-white lg:py-24"><div className="relative mx-auto grid max-w-7xl gap-10 px-5 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-8"><div><p className="text-xs font-black uppercase tracking-[.28em] text-red-100">Steel Frame</p><h2 className="mt-3 max-w-[620px] text-4xl font-black leading-tight sm:text-5xl">Inovação que valoriza seu projeto.</h2><p className="mt-5 max-w-[650px] text-base leading-8 text-red-50/90">Tecnologia construtiva para quem busca eficiência, precisão, organização e desempenho do início ao acabamento.</p><div className="mt-9 grid gap-4 sm:grid-cols-2">{steelBenefits.map(({ icon: Icon, title, text }) => <div key={title} className="rounded-2xl border border-white/15 bg-white/10 p-5"><div className="flex items-start gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-white text-red-600 shadow-lg"><Icon className="h-5 w-5" /></span><div><h3 className="font-black">{title}</h3><p className="mt-1 text-sm leading-6 text-red-50/85">{text}</p></div></div></div>)}</div></div><div id="formulario" className="scroll-mt-40 rounded-[28px] border border-white/10 bg-zinc-950/95 p-6 shadow-[0_28px_90px_rgba(0,0,0,.34)] lg:p-8"><div className="flex items-start justify-between gap-5"><div><p className="text-[11px] font-black uppercase tracking-[.28em] text-red-400">SOLICITE SEU ORÇAMENTO</p><h3 className="mt-3 text-2xl font-black leading-tight sm:text-[1.9rem]">Fale com a equipe da Fast Drywall.</h3><p className="mt-2 text-sm leading-6 text-zinc-400">Preencha seus dados e continue para o WhatsApp.</p></div><WhatsAppMark /></div><form onSubmit={submit} className="mt-6 space-y-3.5"><div className="relative"><User className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-400" /><input id="lead-name" required value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Seu nome" className={`${field} pl-11`} /></div><div className="relative"><Phone className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-400" /><input required value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="WhatsApp / contato" className={`${field} pl-11`} /></div><div className="relative"><Mail className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-400" /><input required type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="Seu e-mail" className={`${field} pl-11`} /></div><select required value={form.interest} onChange={(e) => set("interest", e.target.value)} className={field}><option value="" className="text-zinc-900">Selecione seu interesse</option><option className="text-zinc-900">Drywall</option><option className="text-zinc-900">Steel Frame</option><option className="text-zinc-900">Gesso e forros</option><option className="text-zinc-900">Perfis e acessórios</option><option className="text-zinc-900">Orçamento completo</option></select><button type="submit" disabled={!ready} className={`inline-flex w-full items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-black transition ${ready ? "bg-emerald-500 text-white hover:bg-emerald-400" : "cursor-not-allowed bg-white/10 text-zinc-500"}`}><MessageCircle className="h-4 w-4" /> Continuar para o WhatsApp <ArrowRight className="h-4 w-4" /></button></form></div></div></section>
        ...
        <section className="bg-red-600 py-14 text-white"><div className="mx-auto max-w-7xl px-5 text-center lg:px-8"><h2 className="text-3xl font-black">Peça seu orçamento sem compromisso.</h2><p className="mt-2 text-red-50">Encontre os materiais certos para sua obra com atendimento especializado.</p><button type="button" onClick={scrollToForm} className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-7 py-3.5 text-sm font-black text-red-600 shadow-xl">Solicitar orçamento <ArrowRight className="h-4 w-4" /></button></div></section>

        <section id="localizacao" className="bg-zinc-950 py-20 text-white"><div className="mx-auto grid max-w-7xl gap-8 px-5 lg:grid-cols-[.78fr_1.22fr] lg:items-stretch lg:px-8"><div className="rounded-3xl border border-white/10 bg-white/[.04] p-7 shadow-2xl lg:p-8"><p className="text-xs font-black uppercase tracking-[.28em] text-red-400">Onde estamos</p><h2 className="mt-3 text-4xl font-black">Visite a Fast Drywall.</h2><div className="mt-8 flex gap-4 rounded-2xl border border-white/10 bg-black/20 p-5"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-red-600/15 text-red-400"><MapPin className="h-5 w-5" /></span><p className="text-sm leading-7 text-zinc-300">Av. Luís Eduardo Magalhães, Quadra 05<br />Recreio · Vitória da Conquista - BA<br />CEP 45020-282</p></div><div className="mt-4 flex items-center gap-4 rounded-2xl border border-white/10 bg-black/20 p-5"><Phone className="h-5 w-5 text-red-400" /><p className="text-base font-black text-white">(77) 98856-0246</p></div><a href={maps} onClick={(event) => handleExternal(event, maps)} className="mt-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[.05] px-5 py-3 text-sm font-black">Abrir no Google Maps <ArrowRight className="h-4 w-4" /></a></div><iframe title="Mapa da Fast Drywall em Vitória da Conquista" src="https://www.google.com/maps?q=Av.+Lu%C3%ADs+Eduardo+Magalh%C3%A3es,+Quadra+05,+Recreio,+Vitória+da+Conquista+-+BA&output=embed" className="h-[460px] w-full rounded-3xl border-0" loading="lazy" /></div></section>
      </main>

      <footer className="border-t border-zinc-200 bg-white py-10 text-zinc-900"><div className="mx-auto flex max-w-7xl flex-col gap-7 px-5 lg:flex-row lg:items-center lg:justify-between lg:px-8"><a href="#inicio" aria-label="Voltar ao início" className="shrink-0"><Brand footer /></a><div className="flex items-center gap-3"><a href={instagram} onClick={(event) => handleExternal(event, instagram)} aria-label="Instagram" className="grid h-12 w-12 place-items-center rounded-full bg-gradient-to-br from-violet-600 via-pink-500 to-orange-400 text-white shadow-lg"><Instagram className="h-5 w-5" /></a><a href={facebook} onClick={(event) => handleExternal(event, facebook)} aria-label="Facebook" className="grid h-12 w-12 place-items-center rounded-full bg-[#1877F2] text-white shadow-lg"><Facebook className="h-5 w-5" /></a><button type="button" onClick={scrollToForm} aria-label="WhatsApp" className="grid h-12 w-12 place-items-center rounded-full bg-[#25D366] text-white shadow-lg"><Phone className="h-5 w-5 -rotate-12" /></button><a href={maps} onClick={(event) => handleExternal(event, maps)} aria-label="Google Maps" className="relative grid h-12 w-12 place-items-center overflow-hidden rounded-full bg-white shadow-lg ring-1 ring-zinc-200"><MapPin className="relative z-10 h-5 w-5 text-red-500" /></a></div></div></footer>
    </div>
  );
}
